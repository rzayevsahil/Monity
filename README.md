...
*Ana dashboard - günlük kullanım özeti, ısı haritası ve uygulama listesi*

![İstatistikler](docs/images/statistics.png)
*İstatistikler sayfası - dönemsel analiz ve grafikler*

![Tarayıcı Takibi](docs/images/browser-tracking.png)
*Tarayıcı takibi - site bazlı kullanım analizi*

![Ayarlar](docs/images/settings.png)
*Ayarlar sayfası - tema, dil ve diğer tercihler*

![Başarılar](docs/images/achievements.png)
*Başarılar sayfası - kazanılan ödüller ve ilerleme durumu*

![Hakkında](docs/images/about.png)
*Hakkında sayfası - uygulama bilgileri ve geliştirici iletişimi*
...
- **Tarayıcı takibi:** Chrome, Firefox, Edge, Opera GX gibi tarayıcılarda site bazlı kullanım takibi.
...
- **Single instance kontrolü:** Aynı anda sadece bir uygulama instance'ı çalışabilir.
- **Akıllı Öneri Sistemi:** Kullanım alışkanlıklarını analiz ederek verimli saatler, artış trendleri ve sıra dışı aktiviteler hakkında içgörüler sunar.
- **Günlük Rapor Bildirimi:** Her akşam belirlenen saatte o günkü aktiflik süresini ve en çok kullanılan uygulamayı bildirim olarak sunar.
- **Hedef Sistemi:** Günlük veya haftalık kullanım limitleri belirleme (örn. "Günde max 3 saat Sosyal medya"). Doğal dil işleme ile kolay hedef girişi.
- **Başarım (Achievement) Sistemi:** Sağlıklı dijital alışkanlıklar ve odaklanma süreleri için ödüller kazanma (örn. "Sarsılmaz El", "Derin Odaklanma").

### Arayüz
- **Dashboard:** Seçilen gün için **bugün başlangıç** (ilk kullanım saati veya "—"), toplam süre, kullanım kaydı sayısı, “şu an aktif” uygulama; **tarih** ve **kategori** filtresi, yenile butonu. Aktif hedeflerin ilerleme durumları (progress bar) ana ekranda takip edilebilir.
...
- **İstatistikler:** Ana menüden erişilen ayrı sayfa:
  ...
  - **Dışa aktarma:** Seçilen dönem ve kategoriye göre istatistikleri CSV (UTF-8 BOM, Excel uyumlu) olarak dışa aktarma; özet, uygulama listesi ve günlük dağılım dahil.
  - **Dashboard’a dön:** Sayfa başlığının yanında ve sayfa sonunda geri dönüş butonu.
- **Pomodoro (Odak Zamanlayıcı):** Ana menüden erişilen sayfa; çalışma ve mola süreleri (varsayılan 25/5 dk) ayarlanabilir, geri sayım, başlat/duraklat/durdur ve dönem bitiminde tray bildirimi.
- **Başarılar:** Ana menüden erişilen, kazanılan ödüllerin ve ilerlemenin takip edildiği özel sayfa.
- **Paylaş:** Bugün, bu hafta veya bu ay için kullanım özeti kartı oluşturma. Kartta dönem etiketi, tarih aralığı, toplam süre, önceki döneme göre trend (yüzde), günlük ortalama, en çok kullanılan uygulama, en verimli saatler (varsa), tek içgörü vurgusu ve hedef durumu (varsa) yer alır. Sağ panelde kategori dağılımı (top 3 kategori), top 3 uygulama (mini çubuk grafikle) ve öne çıkanlar listelenir. Görsel sade ve tek odaklı tasarımla sunulur; görüntü panoya kopyalanabilir veya dışarı paylaşılabilir.
- **Tarayıcı Takibi:** Ana menüden erişilen ayrı sayfa:
  - **Saatlik tarayıcı aktivitesi:** Seçilen gün için saatlik kullanım grafiği.
  - **Tarayıcı kullanımı:** Tarayıcı bazında filtreleme ve toplam süre gösterimi.
  - **En çok kullanılan siteler:** Site bazlı kullanım süresi ve yüzde grafiği.
  - **Site kullanım listesi:** Detaylı tablo görünümü, arama ve sıralama özelliği.
- **Ekran görüntüsü alma:** Üst çubuktaki kamera ikonundan tüm sayfa için ekran görüntüsü alabilirsiniz. **Görünen kısım** veya **tam sayfa (kaydırılabilir içerik)** seçenekleri, önizleme, PNG olarak kaydetme ve panoya kopyalama desteği vardır.
- **Ayarlar:**
  ...
  - **Pencere başlığını kaydet:** Açıkken hangi dosya veya sayfa üzerinde çalışıldığı (pencere başlığı) raporlarda kullanılır; gizlilik için kapatılabilir.
  - **Odak modu:** Açıkken, listeden seçilen uygulamalara geçildiğinde tray’de uyarı bildirimi verilir.
  ...
  - **Akıllı Öneri Sistemi:** Kullanım alışkanlıklarını analiz ederek içgörüler sunar; ayarlar sayfasından aktif/pasif edilebilir.
  - **Günlük Rapor Bildirimi:** Belirlenen saatte günlük özet bildirimi; açılır liste (ComboBox) ile güvenli saat seçimi.
  - **Hedef Yönetimi:** Kullanım hedefleri ekleme ve silme. "Günde max 2 saat Sosyal", "Haftalık min 10 saat Geliştirme" gibi cümlelerle hızlı giriş.
  - **Veri yönetimi:** 30 / 90 / 365 günden eski verileri sil veya tüm verileri sil (onay dialogu ile).
  - **Hakkında:** Ana menüden erişilen ayrı sayfa; uygulama logosu, açıklama, özellikler, geliştirici bilgileri, iletişim linkleri (Email, GitHub, LinkedIn) ve versiyon geçmişi.
- **Üst çubuk (header):** Logo ve uygulama adı solda, navigasyon butonları sağda iki sütunlu düzenle yer alır; küçük pencere modunda çakışma olmaz. Butonlar kompakt padding ile daha az yer kaplar.
...
| `browser_sessions` | Tarayıcı oturumları: browser_name, domain, started_at, ended_at, duration_seconds, day_date. |
| `browser_daily_summary` | Günlük tarayıcı özeti: domain, date, total_seconds, session_count. |
...
| `achievements`   | Başarım tanımları: key, type, goal_value. |
| `user_achievements` | Kullanıcı ilerlemesi: achievement_id, current_value, is_unlocked, unlocked_at. |
...
| **Pencere başlığını kaydet** | Açıkken pencere başlığı raporlarda kullanılır; gizlilik için kapatılabilir. |
| **Odak modu** | Açıkken listedeki uygulamalara geçişte tray uyarı bildirimi. |
